...
def palabra_Idioma_interfaz():
    ...
    lbl_palabras["text"]="Ingrese palabra, idioma y\
                          elija una opción: "
...
def func_relacion_hermana():
    ...
    lbl_res_hermanas["text"]=conocer_resultados(
                                                p1,p2,1
                                                ) #En esta se pone el resultado de la búsqueda V/F
    
    ventana_detalles(
                    [conocer_resultados(p1,p2,1),
                     "& padre(Y,P)  & (X!=Y)",
                     "hermano(X,Y) <= padre(X,P)",
                     "Consulta:","Padres Cargados..."]
                    )
...
def func_relacion_hija():
    ...
    palabra1.delete(0, END)
    palabra2.delete(0, END)
    lbl_res_hija["text"]=conocer_resultados(
                                            p1,p2,3
                                            ) #En esta se pone el resultado de la búsqueda V/F
    ventana_detalles(
                    [conocer_resultados(p1,p2,3),
                     "hijo(X,Y) <= padre(X,Y)",
                     "Consulta:","Padres Cargados..."]
                    )
...
def func_relacion_tia():
    ...
    palabra1.delete(0, END)
    palabra2.delete(0, END)
    lbl_res_tia["text"]=conocer_resultados(
                                          p1,p2,4
                                          ) #En esta se pone el resultado de la búsqueda V/F
    ventana_detalles(
                    [conocer_resultados(p1,p2,4),
...
def func_relacion_prima():
    ...
    palabra1.delete(0, END)
    palabra2.delete(0, END)
    lbl_res_primas["text"]=conocer_resultados(
                                              p1,p2,2
                                              )
    ventana_detalles(
                    [conocer_resultados(p1,p2,2),
...
def func_palabra_idioma():
    ...
    if (cmbox_idioma_pal.get()=="Relacionadas"):
        ...
        lbl_res_relacionado["text"]=conocer_resultados(
                                                        palabra,idioma,6
                                                        )
        ventana_detalles(
                        [conocer_resultados(palabra,idioma,6),
                         "palabra_idioma(X,Y) <= ancestro(X,P) & idioma(P,Y)",
                         "palabra_idioma(X,Y) <= descendiente(X,P) & idioma(P,Y)",
                         "palabra_idioma(X,Y) <= idioma(X,Y)","Consulta:",
                         "Idiomas Cargados","Padres Cargados.."]
                        )
    elif (cmbox_idioma_pal.get()=="Conjunto de palabras"):
        print("aqui")
        palabra=palabra1.get()
        idioma=palabra2.get()
        idiomas = conocer_resultados(
                                    palabra,idioma,5
                                    )
        listbox_palabras.insert(0, idiomas)
        listbox_palabras.place(x=275,y=200)
        ventana_detalles(
                        [conocer_resultados(palabra,idioma,5),
        ...
        listbox_palabras.place(x=275,y=200)
        ventana_detalles(
                        [conocer_resultados(palabra,idioma,7),
                         "palabra_idioma(X,Y) <= ancestro(X,P) & idioma(P,Y)",
                         "palabra_idioma(X,Y) <= descendiente(X,P) & idioma(P,Y)",
                         "palabra_idioma(X,Y) <= idioma(X,Y)","Consulta:",
                         "Idiomas Cargados","Padres Cargados.."]
...
def func_idiomas():
    ...
    if (cmbox_idioma.get()=="Contar palabras en común"):
        ...
        ventana_detalles(
                        [conocer_resultados(idioma1,idioma2,8),
        ...
        listbox_palabras.place(x=275,y=200)
        ventana_detalles(
                        [conocer_resultados(idioma1,idioma2,9),
                         "(contar_palabras[X,Y] == len_(P)) <= listar_palabras_comunes(X,Y,P)",
                         "contar_palabras_comunes(X,Y) <= idioma(P,X) & idioma(P,Y)","Consulta:",
                         "Idiomas Cargados","Padres Cargados.."]
                        )
        ...
        ventana_detalles(
                        ["& padre(P,Y) & idioma(Y,X)",
                         "idioma_aportando(A,X,P)<= idioma(P,A) ",
                         "Consulta:","Idiomas Cargados","Padres Cargados.."]
                        )
    else:
        palabra=palabra1.get()
...
def ventana_detalles(
    ...
    detalles["bg"]="black"
    listbox_detalles = Listbox(
                            detalles,width=55, height=15
                            )
    for i  in lista:
...
lbl_bienvenido = Label( ventana_principal, text="Bienvenido",
                        font=("Bitstream Vera Serif", 30),fg="goldenrod",
                        bg="black")
cmbox_opcion_value = StringVar()
cmbox_opcion = ttk.Combobox(ventana_principal,
                            textvariable=cmbox_opcion_value,
                            font=("Helvetica", 12))
cmbox_opcion['values'] =("Dos palabras","Palabra-Idioma","Idiomas")
cmbox_opcion.current(0)
btn_opcion=Button(ventana_principal,text="LISTO",fg="black",
                  bg="goldenrod",font=("Bitstream Vera Serif", 10),
                  highlightbackground ="goldenrod",highlightcolor ="goldenrod",
                  command=opciones)
btn_opcion.config(width=17)
lbl_opcion=Label( ventana_principal, text="Elija operación: ",
                  font=("Helvetica", 12),fg="goldenrod",bg="black")
lbl_opcion.config( width=15)
lbl_relacion=Label( ventana_principal, text="Elija relación: ",
                    font=("Helvetica", 12),fg="goldenrod",bg="black")
lbl_relacion.config( width=15)
var = IntVar()
chbox_etymological_origin_of = Checkbutton(ventana_principal,
                                           text="etymological_origin_of",
                                           variable=var, font=("Helvetica", 12),
                                           fg="goldenrod",bg="black",anchor=SW)
chbox_etymological_origin_of.config( width=25)
var1 = IntVar()
chbox_has_derived_form = Checkbutton(ventana_principal,
                                     text="has_derived_form",
                                     variable=var1, font=("Helvetica", 12),
                                     fg="goldenrod",bg="black",anchor=SW)
chbox_has_derived_form.config( width=25)
var2 = IntVar()
chbox_is_derived_from = Checkbutton(ventana_principal,
                                    text=" is_derived_from",
                                    variable=var2, font=("Helvetica", 12),
                                    fg="goldenrod",bg="black",anchor=SW)
chbox_is_derived_from.config( width=25)
var3 = IntVar()
chbox_etymology = Checkbutton(ventana_principal,
                              text=" etymology",
                              variable=var3,
                              font=("Helvetica", 12),
                              fg="goldenrod",bg="black",
                              anchor=SW)
chbox_etymology.config( width=25)
var4 = IntVar()
chbox_etymologically_related = Checkbutton(ventana_principal,
                                           text="etymologically_related",
                                           variable=var4, font=("Helvetica", 12),
                                           fg="goldenrod",bg="black",anchor=SW)
chbox_etymologically_related.config( width=25)
var5 = IntVar()
chbox_derived = Checkbutton(ventana_principal,
                            text="derived", variable=var5,
                            font=("Helvetica", 12),fg="goldenrod",
                            bg="black",anchor=SW)
chbox_derived.config( width=25)
var6 = IntVar()
chbox_etymologically = Checkbutton(ventana_principal,
                                   text=" etymologically",
                                   variable=var6, font=("Helvetica", 12),
                                   fg="goldenrod",bg="black",anchor=SW)
...
lbl_relaciones=Label( ventana_principal,
                      text="Palabras: ",
                      font=("Helvetica", 12),
                      fg="goldenrod",bg="black")
lbl_relaciones.config( width=37)
lbl_palabras=Label( ventana_principal,
                    font=("Helvetica", 12),
                    fg="goldenrod",bg="black")
lbl_hermanas=Label( ventana_principal,
                    text="Hermanas: ",
                    font=("Helvetica", 12),
                    fg="goldenrod",bg="black")
lbl_hermanas.config( width=15)
lbl_hija=Label( ventana_principal,
                text="Hija: ", font=("Helvetica", 12),
                fg="goldenrod",bg="black")
lbl_hija.config( width=15)
lbl_tia=Label( ventana_principal, text="Tía: ",
               font=("Helvetica", 12),
               fg="goldenrod",bg="black")
lbl_tia.config( width=15)       
lbl_primas=Label( ventana_principal, text="Primas: ",
                  font=("Helvetica", 12),
                  fg="goldenrod",bg="black")
lbl_primas.config( width=15)
lbl_grado=Label( ventana_principal, text="Grado: ",
                 font=("Helvetica", 12),fg="goldenrod",
                 bg="black")
lbl_grado.config( width=15)
lbl_res_hermanas=Label( ventana_principal,
                        fg="goldenrod",bg="black")
lbl_res_hermanas.config( width=5)
lbl_res_hija=Label( ventana_principal,
                    fg="goldenrod",bg="black")
lbl_res_hija.config( width=5)
lbl_res_tia=Label( ventana_principal,
                   fg="goldenrod",bg="black")
lbl_res_tia.config( width=5)
lbl_res_primas=Label( ventana_principal,
                      fg="goldenrod",bg="black")
lbl_res_primas.config( width=5)
lbl_res_grado=Label( ventana_principal,
                     fg="goldenrod",bg="black")
lbl_res_grado.config( width=5)
lbl_pal2_her=Label( ventana_principal,
                    fg="goldenrod",bg="black")
lbl_pal2_her.config( width=17)
lbl_pal2_hija=Label( ventana_principal,
                     fg="goldenrod",bg="black")
lbl_pal2_hija.config( width=17)
lbl_pal2_tia=Label( ventana_principal,
                    fg="goldenrod",bg="black")
lbl_pal2_tia.config( width=17)
lbl_pal2_primas=Label( ventana_principal,
                       fg="goldenrod",bg="black")
lbl_pal2_primas.config( width=17)
lbl_pal1_her=Label( ventana_principal,
                    fg="goldenrod",bg="black")
lbl_pal1_her.config( width=17)
lbl_pal1_hija=Label( ventana_principal,
                     fg="goldenrod",bg="black")
lbl_pal1_hija.config( width=17)
lbl_pal1_tia=Label( ventana_principal,
                    fg="goldenrod",bg="black")
lbl_pal1_tia.config( width=17)
lbl_pal1_primas=Label( ventana_principal,
                       fg="goldenrod",bg="black")
lbl_pal1_primas.config( width=17)
btn_rel_her=Button(ventana_principal,text="Relación",
                   fg="black",bg="goldenrod",
                   font=("Bitstream Vera Serif", 10),
                   command=func_relacion_hermana)
btn_rel_hija=Button(ventana_principal,text="Relación",
                    fg="black",bg="goldenrod",
                    font=("Bitstream Vera Serif", 10),
                    command=func_relacion_hija)
btn_rel_tia=Button(ventana_principal,text="Relación",
                   fg="black",bg="goldenrod",
                   font=("Bitstream Vera Serif", 10),
                   command=func_relacion_tia)
btn_rel_prima=Button(ventana_principal,text="Relación",
                     fg="black",bg="goldenrod",
                     font=("Bitstream Vera Serif", 10),
                     command=func_relacion_prima)
btn_rel_prima_grado=Button(ventana_principal,
                           text="Relación prima y grado",
                           fg="black",bg="goldenrod",
                           font=("Bitstream Vera Serif", 10),
                           command=func_relacion_prima_grado)
btn_rel_prima_grado.config( width=37)
"""componentes idioma palabra"""
cmbox_idioma_pal_value = StringVar()
cmbox_idioma_pal= ttk.Combobox(ventana_principal,
                               textvariable=cmbox_idioma_pal_value)
cmbox_idioma_pal['values'] =("Relacionadas",
                             "Conjunto de palabras",
                             "Idiomas relacionados")
cmbox_idioma_pal.current(0)
btn_idioma_pal=Button(ventana_principal,text="Procesar",
                      fg="black",bg="goldenrod",
                      font=("Bitstream Vera Serif", 10),
                      command=func_palabra_idioma)
lbl_res_relacionado=Label( ventana_principal,
                           font=("Helvetica", 12),
                           fg="goldenrod",bg="black")
listbox_palabras = Listbox(ventana_principal,
                           width=40, height=15)
"""componentes Idiomas"""
cmbox_idioma_value = StringVar()
cmbox_idioma= ttk.Combobox(ventana_principal,
                           textvariable=cmbox_idioma_value)
cmbox_idioma['values'] =("Contar palabras en común",
                         "Listar palabras comunes",
                         "Idioma que más aportó","Listar idiomas")
cmbox_idioma.current(0)
btn_idioma=Button(ventana_principal,text="Procesar",
                  fg="black",bg="goldenrod",
                  font=("Bitstream Vera Serif", 10),
                  command=func_palabra_idioma)
lbl_res_cantPal=Label( ventana_principal,
                       font=("Helvetica", 12),
                       fg="goldenrod",bg="black")
btn_idioma=Button(ventana_principal,text="Procesar",
                  fg="black",bg="goldenrod",
